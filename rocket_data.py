...
def calculate_fuel_mass(rocket_data: dict, transfer: Transfer, deltaV_override: np.ndarray = None):
    """
    ...
    prop_mass = (rocket_data['propellant_mass'] + rocket_data['empty_mass']) * (1 - 1 / mass_ratio)
...
if __name__ == '__main__':
    ...
    plt.vlines(full_refueling / 1e3, 0, 100, color='black', linestyle='--',
               label=str(np.round(full_refueling / 1e3, 1)) + ' km/s')

    plt.title('Falcon 9 oxidizer refueling capacity')
